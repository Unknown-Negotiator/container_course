...
class ImmunogenEnsembleConfig:
    threshold: float = 2.0  # ADA>=threshold => high immunogenicity
    n_models: int = 5  # ensemble size
    random_state: int = 42
    cv_splits: int = 5
    pca_components_grid: List[int] | None = None  # if None, use a default [10, 25, 50, 75, 100]
    c_grid: List[float] | None = None  # LogisticRegression C values, logspace 1e-3..1e3
...
class ImmunogenEnsembleClassifier:
    def __init__(
        ...
        config: ImmunogenEnsembleConfig,
    ):
        self.models = models
        self.config = config
    ...
    def train_from_embeddings_df(
        ...
        from .data import add_binary_ada
    ...
    def predict_proba_members(self, X: np.ndarray) -> np.ndarray:
        """
        Returns an array of shape [N, n_models] with the predicted
        probabilities of high immunogenicity (class 1) from each ensemble member.
        """
        probs = []
    ...
    def predict_stats(self, X: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Returns (mu, sigma) where:
          mu: mean probability over ensemble members, shape [N,]
          sigma: std over ensemble members, shape [N,]
        """
    ...
    def save(self, path: str) -> None:
        """
        Uses joblib to save a dict with keys:
